...
check_task_value(p::Symmetry, value::Object, data, aux_values) =
    check_task_value(p, value.shape, data, aux_values)
    ...
    return _wrap_aux_values(
        ...
        make_either(
            ...
            [
                (Object(
                        opt[p.output_keys[1]], 
                        (source_value.position[1] + !opt[p.output_keys[2]] * (size(source_value.shape)[1] - size(opt[p.output_keys[1]])[1]), 
            ...
            [
                (Object(
                        ...
                        (source_value.position[1], 
                            source_value.position[2] + !opt[p.output_keys[2]] * (size(source_value.shape)[2] - size(opt[p.output_keys[1]])[2]))
                    ), 
                    opt[p.output_keys[2]], 
                    opt[p.output_keys[3]])
                for opt in unpack_value(res)
...
function from_abstract_value(p::Abstractor{VerticalSymmetry}, source_value::Object, is_left::Bool, keep_pivot_point::Bool)
...
function from_abstract_value(p::Abstractor{HorisontalSymmetry}, source_value::Object, is_top::Bool, keep_pivot_point::Bool)
...
from_abstract_value(p::Abstractor{<:Symmetry}, source_value::AbstractArray{Object}, is_left::AbstractArray{Bool}, keep_pivot_point::AbstractArray{Bool}) =
    Dict(
        p.output_keys[1] => [
            wrap_func_call_value_root(p, from_abstract_value, v, l, k) for (v, l, k) in zip(source_value, is_left, keep_pivot_point)
        ]
    )

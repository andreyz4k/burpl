...
function get_new_solutions_for_input_key(solution, key)
    ...
    interesting_source = all(
        in(solution.field_info[k].derived_from, solution.field_info[key].previous_fields) for
    ...
    for (priority, abstractor) in get_next_operations(solution, key)
        ...
        for matched_solution in match_fields(new_solution)
            pr = priority * get_unmatched_complexity_score(matched_solution) * matched_solution.score^1.5
...
function get_new_solutions_for_unfilled_key(solution::Solution, key::String)
    ...
    priority_key =
        all(in(k, solution.field_info[solution.field_info[key].derived_from].previous_fields) for k in source_fields)

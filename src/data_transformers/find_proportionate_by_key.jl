...
_check_effective_factor_key(shift_key, input_key, taskdata) = all(haskey(task_data, shift_key) for task_data in taskdata) && 
        any(apply_func(task_data[input_key], (x, y) -> x .* y, task_data[shift_key]) != task_data[input_key] for task_data in taskdata)
